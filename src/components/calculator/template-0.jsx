...
import { calculateDistributor } from 'utils/calculator';
import { displayPercentage, displayPrice } from 'utils/number';

export default function Tempalate0() {
  ...
  const handleClickSubmit = (event) => {
    ...
    setCalculation(calculateDistributor(valueBet, valueRatingA, valueRatingB));
  };

  return (
    <div>
      <div className={styles.header}>
        <h3>1 Gagnant, 1 Remboursé</h3>
        <h3>2 Signes</h3>
      </div>
      <Form className={styles.form_container} onSubmit={handleClickSubmit}>
        ...
        <Row className={classNames(styles.form_group, styles.form_result)}>
          <Col md={6}>Nouvelle cote:</Col>
          <Col>{calculation.mNouvelleCote?.toFixed(2)}</Col>
        </Row>
